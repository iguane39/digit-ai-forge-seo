---
id: 48
branche: Discover
noeud: Angles Chauds
volet: TRANSVERSAL
statut_instrumentation: SD
source_requise: "crawl (dates) + recherche web"
doublon_de: null
# --- rempli pendant la mission ---
etat: a-faire
motif_hors_perimetre: null
verdict: null
niveau_preuve: null
date_mesure: null
actions_liees: []
---

# Discover / Angles Chauds

> Volet **TRANSVERSAL** -- statut **SD** (instrumente sans dependance externe)

## Question d'audit

Le site produit-il du contenu sur des sujets à intérêt momentané ?

## Source requise

crawl (dates) + recherche web

## Methode

repérer les contenus liés à l'actualité du secteur et leur fraîcheur ; comparer au rythme du secteur

## Critere de verdict

≥ 1 contenu d'actualité par mois pour un site à ambition Discover, sinon nœud hors périmètre

---

## Constat

<!-- Etape 2 du pipeline. Ce qui est, mesure. Chaque chiffre porte son
     niveau de preuve : [T1 observe] [T2 declare] [T3 tiers] [T4 infere].
     Si non mesurable : le dire et renseigner motif_hors_perimetre. -->

## Preuves

<!-- Ou la mesure a ete prise : URL, fichier d'export et periode, requete,
     date de consultation. Verifiable par un tiers. -->

## Interpretation

<!-- Etape 3 du pipeline. Le mecanisme : comment ce constat coute du trafic
     ou des leads. "Ce n'est pas optimal" n'est pas un mecanisme. -->
