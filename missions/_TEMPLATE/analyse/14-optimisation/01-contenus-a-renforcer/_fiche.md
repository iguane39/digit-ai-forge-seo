---
id: 68
branche: Optimisation
noeud: Contenus À Renforcer
volet: TRANSVERSAL
statut_instrumentation: EX
source_requise: "GSC (positions 5-20 à fort volume d'impressions)"
doublon_de: null
# --- rempli pendant la mission ---
etat: a-faire
motif_hors_perimetre: null
verdict: null
niveau_preuve: null
date_mesure: null
actions_liees: []
---

# Optimisation / Contenus À Renforcer

> Volet **TRANSVERSAL** -- statut **EX** (instrumente si export fourni (GSC / GA / CRM))

## Question d'audit

Quelles pages sont proches du seuil de performance ?

## Source requise

GSC (positions 5-20 à fort volume d'impressions)

## Methode

trier par (impressions × gain de CTR attendu si passage en top 3) ; croiser avec la complétude du contenu

## Critere de verdict

liste des 10 pages au plus fort potentiel de gain à effort faible

---

## Constat

<!-- Etape 2 du pipeline. Ce qui est, mesure. Chaque chiffre porte son
     niveau de preuve : [T1 observe] [T2 declare] [T3 tiers] [T4 infere].
     Si non mesurable : le dire et renseigner motif_hors_perimetre. -->

## Preuves

<!-- Ou la mesure a ete prise : URL, fichier d'export et periode, requete,
     date de consultation. Verifiable par un tiers. -->

## Interpretation

<!-- Etape 3 du pipeline. Le mecanisme : comment ce constat coute du trafic
     ou des leads. "Ce n'est pas optimal" n'est pas un mecanisme. -->
