---
id: 70
branche: Optimisation
noeud: Pages À Supprimer
volet: TRANSVERSAL
statut_instrumentation: EX
source_requise: "GSC + GA ; à défaut signaux structurels"
doublon_de: null
# --- rempli pendant la mission ---
etat: a-faire
motif_hors_perimetre: null
verdict: null
niveau_preuve: null
date_mesure: null
actions_liees: []
---

# Optimisation / Pages À Supprimer

> Volet **TRANSVERSAL** -- statut **EX** (instrumente si export fourni (GSC / GA / CRM))

## Question d'audit

Quelles pages n'apportent rien (0 trafic, 0 lien, 0 conversion) ?

## Source requise

GSC + GA ; à défaut signaux structurels

## Methode

croiser absence de clics sur 12 mois, absence de liens entrants et absence de conversion

## Critere de verdict

candidates listées avec, pour chacune, l'arbitrage suppression / fusion / amélioration — **jamais de suppression recommandée sur la seule absence de trafic**

---

## Constat

<!-- Etape 2 du pipeline. Ce qui est, mesure. Chaque chiffre porte son
     niveau de preuve : [T1 observe] [T2 declare] [T3 tiers] [T4 infere].
     Si non mesurable : le dire et renseigner motif_hors_perimetre. -->

## Preuves

<!-- Ou la mesure a ete prise : URL, fichier d'export et periode, requete,
     date de consultation. Verifiable par un tiers. -->

## Interpretation

<!-- Etape 3 du pipeline. Le mecanisme : comment ce constat coute du trafic
     ou des leads. "Ce n'est pas optimal" n'est pas un mecanisme. -->
