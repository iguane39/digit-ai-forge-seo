---
id: 77
branche: Croissance
noeud: Actifs Qui Composent
volet: STRATEGIE
statut_instrumentation: SD
source_requise: "ensemble de l'audit"
doublon_de: null
# --- rempli pendant la mission ---
etat: a-faire
motif_hors_perimetre: null
verdict: null
niveau_preuve: null
date_mesure: null
actions_liees: []
---

# Croissance / Actifs Qui Composent

> Volet **STRATEGIE** -- statut **SD** (instrumente sans dependance externe)

## Question d'audit

Quels actifs produisent un rendement cumulatif plutôt que ponctuel ?

## Source requise

ensemble de l'audit

## Methode

distinguer les actions à effet unique (correction technique) des actifs à effet cumulé (pilier qui capte la traîne, outil gratuit qui attire des liens, données propriétaires)

## Critere de verdict

≥ 3 actifs cumulatifs identifiés dans la trajectoire

---

## Constat

<!-- Etape 2 du pipeline. Ce qui est, mesure. Chaque chiffre porte son
     niveau de preuve : [T1 observe] [T2 declare] [T3 tiers] [T4 infere].
     Si non mesurable : le dire et renseigner motif_hors_perimetre. -->

## Preuves

<!-- Ou la mesure a ete prise : URL, fichier d'export et periode, requete,
     date de consultation. Verifiable par un tiers. -->

## Interpretation

<!-- Etape 3 du pipeline. Le mecanisme : comment ce constat coute du trafic
     ou des leads. "Ce n'est pas optimal" n'est pas un mecanisme. -->
