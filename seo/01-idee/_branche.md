---
rang: 1
branche: Idée
volet_dominant: STRATEGIE
nb_noeuds: 5
---

# 01. Idée

5 noeuds. Volet dominant : **STRATEGIE**.

| # | Noeud | Volet | Statut | Dossier |
|---|---|---|---|---|
| 1 | Niche | STRATEGIE | `SD` | `01-niche/` |
| 2 | Intention De Recherche | STRATEGIE | `SD` | `02-intention-de-recherche/` |
| 3 | Potentiel Business | STRATEGIE | `PY` | `03-potentiel-business/` |
| 4 | Concurrence SERP | STRATEGIE | `SD` | `04-concurrence-serp/` |
| 5 | Opportunité Cachée | STRATEGIE | `EX` | `05-opportunite-cachee/` |

> Referentiel canonique de la forge, en lecture seule. Le travail se fait
> dans le dossier `seo/analyse/` du projet audite.
