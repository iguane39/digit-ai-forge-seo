---
id: 42
branche: Autorité
noeud: Liens Déjà Visibles
volet: TRANSVERSAL
statut_instrumentation: SD
source_requise: "recherche web + crawl des 404"
doublon_de: null
# --- rempli pendant la mission ---
etat: a-faire
motif_hors_perimetre: null
verdict: null
niveau_preuve: null
date_mesure: null
actions_liees: []
---

# Autorité / Liens Déjà Visibles

> Volet **TRANSVERSAL** -- statut **SD** (instrumente sans dependance externe)

## Question d'audit

Quels liens existants sont récupérables (mentions non liées, liens vers 404) ?

## Source requise

recherche web + crawl des 404

## Methode

croiser les mentions du nœud 41 sans lien sortant, et les URLs en 404 recevant des liens

## Critere de verdict

liste nominative d'opportunités, chiffrée en nombre de domaines

---

## Constat

<!-- Etape 2 du pipeline. Ce qui est, mesure. Chaque chiffre porte son
     niveau de preuve : [T1 observe] [T2 declare] [T3 tiers] [T4 infere].
     Si non mesurable : le dire et renseigner motif_hors_perimetre. -->

## Preuves

<!-- Ou la mesure a ete prise : URL, fichier d'export et periode, requete,
     date de consultation. Verifiable par un tiers. -->

## Interpretation

<!-- Etape 3 du pipeline. Le mecanisme : comment ce constat coute du trafic
     ou des leads. "Ce n'est pas optimal" n'est pas un mecanisme. -->
