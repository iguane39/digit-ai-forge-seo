---
id: 49
branche: Discover
noeud: Titres Qui Attirent
volet: TRANSVERSAL
statut_instrumentation: SD
source_requise: "crawl (balises `title`, H1)"
doublon_de: null
# --- rempli pendant la mission ---
etat: a-faire
motif_hors_perimetre: null
verdict: null
niveau_preuve: null
date_mesure: null
actions_liees: []
---

# Discover / Titres Qui Attirent

> Volet **TRANSVERSAL** -- statut **SD** (instrumente sans dependance externe)

## Question d'audit

Les titres sont-ils formulés pour le clic sans être trompeurs ?

## Source requise

crawl (balises `title`, H1)

## Methode

échantillon de 20 titres : longueur, présence d'un bénéfice ou d'une tension, unicité, cohérence avec le contenu réel

## Critere de verdict

0 titre dupliqué · 0 promesse non tenue par la page · longueur adaptée à l'affichage SERP

---

## Constat

<!-- Etape 2 du pipeline. Ce qui est, mesure. Chaque chiffre porte son
     niveau de preuve : [T1 observe] [T2 declare] [T3 tiers] [T4 infere].
     Si non mesurable : le dire et renseigner motif_hors_perimetre. -->

## Preuves

<!-- Ou la mesure a ete prise : URL, fichier d'export et periode, requete,
     date de consultation. Verifiable par un tiers. -->

## Interpretation

<!-- Etape 3 du pipeline. Le mecanisme : comment ce constat coute du trafic
     ou des leads. "Ce n'est pas optimal" n'est pas un mecanisme. -->
